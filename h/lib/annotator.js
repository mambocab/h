...
// Generated by CoffeeScript 1.6.3
(function() {
  var $, Annotator, Delegator, DummyDocumentAccess, Highlight, LinkParser, Range, Util, findChild, fn, functions, g, getNodeName, getNodePosition, gettext, simpleXPathJQuery, simpleXPathPure, util, _Annotator, _gettext, _i, _j, _len, _len1, _ref, _ref1, _t,
  ...
  Annotator = (function(_super) {
    ...
    Annotator.prototype._selectorCreators = [];

    Annotator.prototype.registerSelectorCreator = function(selectorCreator) {
      return this._selectorCreators.push(selectorCreator);
    };

    Annotator.prototype._anchoringStrategies = [];

    Annotator.prototype.registerAnchoringStrategy = function(strategy) {
    ...
    Annotator.prototype._highlighters = [];

    Annotator.prototype.registerHighlighter = function(highlighter) {
    ...
    Annotator.prototype._setupAnchorEvents = function() {
      ...
      return this.on('annotationUpdated', function(annotation) {
        var anchor, index, _k, _len2, _ref1, _results;
        _ref1 = annotation.anchors || [];
        _results = [];
        for (_k = 0, _len2 = _ref1.length; _k < _len2; _k++) {
          anchor = _ref1[_k];
          _results.push((function() {
            var _l, _ref2, _ref3, _ref4, _results1;
            _results1 = [];
            for (index = _l = _ref2 = anchor.startPage, _ref3 = anchor.endPage; _ref2 <= _ref3 ? _l <= _ref3 : _l >= _ref3; index = _ref2 <= _ref3 ? ++_l : --_l) {
              _results1.push((_ref4 = anchor.highlight[index]) != null ? _ref4.annotationUpdated() : void 0);
            }
            return _results1;
          })());
        }
    ...
    Annotator.prototype._createAnchorWithStrategies = function(annotation, target, strategies, promise) {
      ...
      try {
        ...
        iteration.then(function(anchor) {
          ...
          Util.removeFromSet(annotation, _this.orphans);
          if (annotation.anchors.length === annotation.target.length) {
            Util.removeFromSet(annotation, _this.halfOrphans);
          } else {
            Util.addToSet(annotation, _this.halfOrphans);
          }
    ...
    Annotator.prototype._realizeAnchor = function(anchor, page) {
      var created, error, p, pagesTodo, promises, renderedPages, _k, _l, _len2, _ref1, _ref2, _ref3, _results,
        _this = this;
      ...
      renderedPages = (function() {
        _results = [];
        for (var _k = _ref1 = anchor.startPage, _ref2 = anchor.endPage; _ref1 <= _ref2 ? _k <= _ref2 : _k >= _ref2; _ref1 <= _ref2 ? _k++ : _k--){ _results.push(_k); }
        return _results;
      ...
      try {
        ...
        (_ref3 = Annotator.$).when.apply(_ref3, promises).always(function() {
          ...
          if (created.length) {
            return _this.publish('highlightsCreated', created);
          }
        });
      } catch (_error) {
        error = _error;
    ...
    Annotator.prototype._virtualizeAnchor = function(anchor, pageIndex) {
      var error, highlight;
    ...
    Annotator.prototype._verifyAnchor = function(anchor, reason, data) {
      var dfd, error,
        _this = this;
      dfd = Annotator.$.Deferred();
    ...
    Annotator.prototype._removeAnchor = function(anchor) {
      var anchors, annotation, index, _k, _ref1, _ref2;
      for (index = _k = _ref1 = anchor.startPage, _ref2 = anchor.endPage; _ref1 <= _ref2 ? _k <= _ref2 : _k >= _ref2; index = _ref1 <= _ref2 ? ++_k : --_k) {
      ...
      if (annotation.anchors.length) {
        return Util.addToSet(annotation, this.halfOrphans);
      } else {
        Util.addToSet(annotation, this.orphans);
        return Util.removeFromSet(annotation, this.halfOrphans);
      }
    };
    ...
    Annotator.prototype._getTargetFromSelection = function(selection) {
      var c, dfd, error, promises, selectors, _ref1,
        _this = this;
      dfd = Annotator.$.Deferred();
      selectors = [];
      promises = (function() {
        var _k, _len2, _ref1, _results;
        _ref1 = this._selectorCreators;
        _results = [];
        for (_k = 0, _len2 = _ref1.length; _k < _len2; _k++) {
          c = _ref1[_k];
          try {
            _results.push(c.describe(selection).then(function(description) {
              var selector, _l, _len3, _results1;
              _results1 = [];
    ...
    Annotator.prototype._getTargetsFromSelections = function(selections) {
      var dfd, promises, selection, targets, _ref1,
        _this = this;
      dfd = Annotator.$.Deferred();
      targets = {};
      promises = (function() {
        var _k, _len2, _results;
        _results = [];
        for (_k = 0, _len2 = selections.length; _k < _len2; _k++) {
          selection = selections[_k];
    ...
    Annotator.prototype.onSuccessfulSelection = function(event, immediate) {
      ...
      this._getTargetsFromSelections(event.segments).then((function(targets) {
        _this.selectedTargets = targets;
        _this.selectedData = event.annotationData;
        if (immediate) {
          _this.onAdderClick(event);
          return dfd.resolve("adder clicked");
        } else {
          _this.adder.css(util.mousePosition(event, _this.wrapper[0])).show();
    ...
    Annotator.prototype._createHighlightUsingHighlighters = function(anchor, page, highlighters, promise) {
    ...
    Annotator.prototype._createHighlight = function(anchor, page) {
      var dfd;
      dfd = Annotator.$.Deferred();
    ...
    Annotator.prototype._realizePage = function(index) {
      var anchor, _k, _len2, _ref1, _ref2;
    ...
    Annotator.prototype.onHighlightMousedown = function(event) {};
